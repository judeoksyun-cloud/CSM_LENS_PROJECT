...
| 03 | 보험부채 변동내역 | 계리적 가정 변경의 BEL·RA·CSM 영향; 연말에만 표시 |
| 04 | 예실차 · 5% 관리기준 | 별도 SAP의 보험금·사업비 예실차 비율. 전체보기에서 금액·비율·전년비 확인 |
| 05 | CSM 추이 전망 | 과거 연도말/최신 분기 실적과 향후 1~5년의 연도별 Base/Worst 전망 |
| 참고 | 예실차 (공시기준) | Open DART 연결재무제표 주석의 공시 예실차 |
| 참고 | 경과기간 손해율·유지비율 | 본문 탭에서 손해율 또는 유지비율을 선택하고 미니 흐름·전체 공시표 확인 |
| DATA | 데이터 기준 | 원본, 파싱 규칙, 검증값, 허용오차, 선택 데이터 감사 이력 |
...
- 보험손익은 DART 표준 계정 ID `ifrs-full_InsuranceServiceResult`, 지배주주 순이익은 `ifrs-full_ProfitLossAttributableToOwnersOfParent`를 우선해 저장한다. 한글 계정명은 표준 ID가 없을 때만 사용한다.
- 분기 단독 손익은 DART 당 3개월 값을 사용하고 누적 차감값과 0.001십억원 허용오차로 대사한다.
...
2026~2030년을 빠짐없이 표시하며 전년도 기말을 다음 연도 기시로 연결한다.

### Base

- 전년 하반기 계절성과 당해 상반기 전년동기 성장 신호를 당해연도 신계약 전망에 사용한다. 장기 신계약 추세율은 2024·2025년 실적과 2026년 Base 전망에서 계산한 두 변화율을 35%·65% 가중하고 -5%~+5%로 제한한다.
...
- 신계약 CSM: Q3~Q4 잔여 신계약 전망을 최종 Base 대비 10% 감소
- CSM 조정: Q3~Q4 잔여 조정을 최종 Base 대비 10% 악화. Base 조정 부담이 매우 작을 때만 기시 CSM의 1% 금액을 최소 하방으로 사용하며, 경영목표 연결분이 있으면 이를 포함한 최종 Base 조정액에서 계산
- 상반기 확정 실적은 낮추지 않고, 이자부리·상각은 스트레스된 CSM 규모에 맞춰 재계산

롤링 백테스트 54건은 Worst 하방률 산식이 아니라 모델 신뢰도와 검증 제한 라벨을 산정하는 참고 표본으로 보존한다.
- 이자부리·상각: 스트레스 이후 CSM 규모에 기존 기계 비율을 다시 적용

상세 팝업은 1~5년의 연도별 전망값과 구간별 근거를 임원보고용 문장으로 표시하고, 하단에 회사별 판단 근거와 사용한 애널리스트 보고서를 연결한다.
...
```bash
python tools/quarterly_dart_insurance_extract.py --company samsung-life --period 2026-q2 --force
```
...
- 분기 단독값 합계와 연간 누적값
- 표준 계정 ID, DART 당 3개월 직접값과 누적 차감값의 차이
...
- CSM 전망의 Base/Worst 산식, 1·2·3·4·5년 표시 시점
...
2. 8개 탭의 독립 페이지 전환
3. 9개사 카드와 표 렌더링
4. CSM 추이 상세 팝업의 1·2·3·4·5년 Base/Worst 값
...
8. Worst 신계약과 조정은 전 보험사 공통 하방률(신계약 CSM -10%, CSM 조정 10% 악화)을 사용하고 회사별 임의 숫자로 바꾸지 않는다. Base 조정 부담이 매우 작은 경우에만 기시 CSM의 1% 금액을 최소 하방으로 사용한다.
