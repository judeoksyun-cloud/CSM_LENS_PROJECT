...
분기 손익 계정은 표시명보다 DART 표준 계정 ID를 우선한다. 보험손익은 `ifrs-full_InsuranceServiceResult`, 지배주주 순이익은 `ifrs-full_ProfitLossAttributableToOwnersOfParent`를 사용하고, 당 3개월 직접 공시값과 누적 차감값을 함께 보존해 상호 검증한다.
...
- 전망: 2026·2027·2028·2029·2030년 Base/Worst 연도별 시나리오
...
- 현재 최신 실적이 2026년 2분기까지 있으므로 전망 기준은 2026년 2분기말이다. 2026년 전망은 상반기 확정 실적을 잠그고 Q3~Q4만 추정한다.
- 2025년 1분기, 2분기처럼 이미 실적값이 있는 과거 분기의 전망은 노출하지 않는다.
- 최신 검증 기간이 바뀌면 전망 기준도 자동으로 바뀐다.
- 전망 CSM을 보여 줄 때는 Movement 전망도 함께 보여 준다.
- 대시보드에는 전망 시점을 명시한다.
- Base는 전년 계절성과 당해 성장 신호를 사용한다. 장기 신계약 추세율은 2024·2025년 실적과 2026년 Base 전망의 변화율을 35%·65% 가중한 뒤 성장 상한과 백테스트 통제를 적용한다.
- CSM 조정률은 같은 3개년을 20%·30%·50% 가중하며 연간 순양(+) 조정, 분기성 환입·재분류의 총액 반복과 경영목표 연결분을 제외한다.
- 회사별 직접 증권사 근거가 있는 경우에만 정성 입력을 25% 오버레이한다.
- Worst는 전 보험사 공통 단순 하방률(잔여 신계약 CSM -10%, CSM 조정 10% 악화·최소 기시 CSM 1%p)을 사용한다.
